...
**Relationship to other documents**: this document and `docs/roadmap-product.md` are tracked and ship to contributors. Each repo's `CONTRIBUTING.md` carries the rules a contributor is measured against day to day, and each repo's `.agent/decisions.md` carries the reasoning as ADRs. Where a rule here is stricter than a repo's own file, this document wins. Where the repo's file is stricter, that wins. Never the more lenient one. Where either disagrees with a recorded ADR, the ADR wins, because it is the entry that was reviewed and is permanent.

Maintainer-local planning drafts exist outside version control and are not part of this chain. See `docs/planning/README.md`.

**Audience**: everyone who touches this project, human or agent. For an agent, this document and the repo's `.agent/` files are the context to load at the start of a session.
...
|---|---|---|---|
| Node.js | 22 LTS | `nvm install 22` | TS runtime |
| pnpm | 11+ | `corepack enable` | JS package manager |
...
The Node and pnpm rows moved from 20 and 9+ to 22 and 11+ in August 2026: pnpm 11 requires Node 22.13 or newer, and Node 20 left maintenance in April 2026. Recorded as ADR-012 in `pulsar-app`.

A `verify-toolchain.sh` asserting each minimum is specified here but not yet written. Until it exists, `CONTRIBUTING.md` in each repo lists the version checks to run by hand.
