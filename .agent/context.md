...
Phase F is underway. Block 1 is done: `internal/rpc/client.go` wraps the Soroban RPC surface behind the `Caller` interface and classifies the -32600 range error (ADR-034, ADR-035); `internal/rpc/events.go` is the poller, one goroutine per contract, draining each contract from its stored cursor to the tip and committing the batch and the advanced cursor in one transaction. Paging terminates on an empty or short page, never on the response cursor (ADR-028); the two -32600 cases are told apart by fresh getHealth bounds rather than message text (ADR-036), the low side resuming from the new floor and the high side holding position. `cmd/pulsar-indexer/main.go` now runs the startup pipeline (config, logger, db, migrations, bootstrap registration) and launches a poll loop per contract, stopping cleanly on SIGINT/SIGTERM. This is the poller-startup slice of step 72.

Block 2 is the remaining work: the HTTP surface (steps 62-71) is now complete, leaving the rest of step 72 (wiring the API server alongside the poller, with auth and rate limiting on writes and a read deadline for the whole surface) and the README (step 73). The HTTP handlers are where the SDK wire contracts above are honoured on the serving side.

The HTTP read surface is landing incrementally. The ADR-017 envelope, the request-logging and panic-recovery middleware, `/health`, and the contract routes (`GET`/`POST /contracts`, `GET`/`DELETE /contracts/:id`) are in place, and the events read routes `GET /contracts/:id/events` and `GET /events/:id` (step 68) now implement the SDK's events-query and single-event contracts; ADR-041 records their newest-first paging over an id keyset, the literal case-sensitive topic-substring filter, and the store now carrying its SQL dialect, and ADR-042 records the boundary validation of the `name` and `topic_contains` filters for storable UTF-8. The read-only GraphQL surface (`POST /graphql`, steps 70-71) has now landed; ADR-043 records the `graph-gophers/graphql-go` choice, the read-only scope with `Contract.events` nesting and mutations deferred to step 72's auth, the scalar mapping, the GraphQL `{ data, errors }` envelope carrying the existing catalog codes in `extensions` with no new apierror code, the null-vs-`NOT_FOUND_CONTRACT` not-found semantics, and the security limits with introspection kept on. Still ahead before the API server is mounted on a public listener: authentication and rate limiting on the write routes (`POST`/`DELETE /contracts`) with a contract delete cascading to its events, a read deadline (statement timeout) applied surface-wide when the server is wired per ADR-042 and ADR-043 (which also bounds the GraphQL query-cost amplification the security review flagged), the rest of step 72, and the README (step 73). A pulsar-docs `indexer-reference/graphql-api.md` companion is owed per ADR-009 and ADR-043, to land with the step 72 exposure since the endpoint is not reachable until then.
