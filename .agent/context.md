...
Block 2 is the remaining work: the HTTP surface (steps 62-71), the rest of step 72 (wiring the API server alongside the poller), and the README (step 73). The HTTP handlers are where the SDK wire contracts above are honoured on the serving side.

The HTTP read surface is landing incrementally. The ADR-017 envelope, the request-logging and panic-recovery middleware, `/health`, and the contract routes (`GET`/`POST /contracts`, `GET`/`DELETE /contracts/:id`) are in place, and the events read routes `GET /contracts/:id/events` and `GET /events/:id` (step 68) now implement the SDK's events-query and single-event contracts; ADR-041 records their newest-first paging over an id keyset, the literal case-sensitive topic-substring filter, and the store now carrying its SQL dialect, and ADR-042 records the boundary validation of the `name` and `topic_contains` filters for storable UTF-8. Still ahead before the API server is mounted on a public listener: authentication and rate limiting on the write routes (`POST`/`DELETE /contracts`) with a contract delete cascading to its events, a read deadline (statement timeout) applied when the server is wired per ADR-042, the rest of step 72, and the README (step 73).
